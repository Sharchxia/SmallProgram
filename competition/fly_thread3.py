...
def throw():
    global THROW_FLAG, BACK_FLAG
    while True and not LAND_FLAG and not BACK_FLAG:
        if THROW_FLAG:
            time.sleep(0.5)
            if THROW_TIME == 1:
    ...
    print('\033[0;35m!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
    print('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
    print('!!!!!\033[0mThrow thread has stopped\033[0;35m!!!!!')
    print('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
    print('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\033[0m')

def fly_once(flag):
    global X_HAVE_FLIED, Y_HAVE_FLIED, LAND_FLAG, THROW_FLAG
    one_distance = (REGION_X[flag]**2+REGION_Y[flag]**2)**0.5
    ...
    x_speed = REGION_X[flag]/one_distance*CRUISE_SPEED
    y_speed = REGION_Y[flag]/one_distance*CRUISE_SPEED
    
    have_fly_time = 0

    while have_fly_time < fly_time:
        ...
        if vehicle.location.global_relative_frame.alt < HEIGHT*0.95:
            try:
                ...
                print('Rise to keep height, speed is:', RISE_SPEED)
        ...
        if THROW_FLAG:
            print('\033[0;32m*************************')
            print('*************************')
            print('****\033[0mWait for throwing(3 senconds)\033[0;32m****')
            print('*************************')
            print('*************************\033[0m')
            send_body_ned_velocity(0, 0, 0)
            time.sleep(3)
        ...
        if BACK_FLAG:
            break
        send_body_ned_velocity(x_speed, y_speed, 0)

        have_fly_time += TIME_RANGE
        X_HAVE_FLIED += x_speed * TIME_RANGE
        Y_HAVE_FLIED += y_speed * TIME_RANGE

        print('Speed_\033[0;32mx\033[0m is %f, speed_\033[0;32my\033[0m is %f, speed_\033[0;32mz\033[0m is %f, \033[0;32mhight\033[0m is %f' % (x_speed,y_speed,0,vehicle.location.global_relative_frame.alt))
...
def fly_back():
    ...
    print('\033[0;32mBacing distance is:\033[0m', total, 'Velocity is:', BACK_SPEED)
...
def fly_cruise():
    ...
    for fly_time in range(len(REGION_X)):
...
def load_points():
    ...
    x, y, h =  dic['points_x'], dic['points_y'], dic['height']
    x0, y0 = x[0], y[0]
...
if __name__ == '__main__':
    ...
    REGION_X, REGION_Y, HEIGHT = load_points()
    ...
    if len(ports):
        print('\033[0;32mcom-serial port is:\033[0m','/dev/artpi')
        com_port = serial.Serial('/dev/artpi', 9600, timeout=0.2)
        print('\033[0;32mvehicle port is:\033[0m', '/dev/feikong')
        ...
        try:
            ...
            throw_thread = Thread(target=throw)
            detect_thread.start()
            throw_thread.start()
            ...
            vehicle.mode = VehicleMode('LAND')
            print('\033[0;31mLanding because of Exception\033[0m')
            BACK_FLAG = 1
            LAND_FLAG = 1
            pwm.ChangeDutyCycle(2)
            time.sleep(1)
